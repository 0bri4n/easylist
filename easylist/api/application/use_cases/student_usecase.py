...
from easylist.api.application.dtos.student_dto import StudentCreateDTO, StudentReadDTO, StudentUpdateDTO
...
class StudentUseCase:
    ...
    def get_student(self, student_id: str) -> StudentReadDTO | None:
        student = self._student_repository.get_by_id(student_id)
        return StudentReadDTO.model_validate(student) if student else None

    def update_student(self, student_id: str, student_data: StudentUpdateDTO) -> StudentReadDTO | None:
        updated_student = self._student_repository.update(str(student_id), student_data.model_dump(exclude_unset=True))
        if updated_student:
    ...
    def delete_student(self, student_id: str) -> bool:
        deleted = self._student_repository.delete(str(student_id))
        if deleted:
